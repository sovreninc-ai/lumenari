...
async function submit(email: string): Promise<{
  ok: boolean;
  message: string;
}> {
  try {
    ...
    return {
      ok: true,
      message:
        data.message ?? "Check your email — we sent a confirmation link.",
    };
  } catch {
    return { ok: false, message: "Couldn't reach our server. Try again." };
  }
}
    ...
    >
      <div className="relative w-full max-w-md rounded-3xl bg-white shadow-2xl p-8">
        ...
        <h2 id="exit-intent-title" className="display text-2xl mb-3">
          Get a free kit on us.
        </h2>
        <p className="text-[var(--muted)] mb-5 leading-relaxed">
          Subscribe to the Lumenari newsletter and we&apos;ll send your first
          kit free with the welcome email. One useful email a week, no fluff.
        </p>
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          >
            {status === "loading" ? "Subscribing..." : "Send my free kit"}
          </button>
          {message ? (
            <p
              role="status"
