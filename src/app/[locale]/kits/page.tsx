    ...
    <div className="mx-auto max-w-6xl px-6 py-20">
      <div className="mb-10">
        ...
        <h1 className="display text-4xl sm:text-5xl mt-2 mb-3">
          Pick a kit. Or grab a bundle.
        </h1>
        <p className="text-[var(--muted)] max-w-xl">
          Each kit is a single download — a SKILL.md, a ChatGPT-ready system
          prompt, and a stack of supporting files you drop into your AI tool
          of choice.
        </p>
      </div>

      <h2 className="eyebrow mb-4">Individual kits</h2>
      ...
      <h2 className="eyebrow mt-20 mb-4">Featured bundles</h2>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {featured.map((b) => (
          <BundleCard key={b.slug} bundleSlug={b.slug} />
        ))}
      </div>

      {others.length > 0 ? (
        <>
          <h2 className="eyebrow mt-16 mb-4">More bundles</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {others.map((b) => (
              <BundleCard key={b.slug} bundleSlug={b.slug} variant="compact" />
            ))}
          </div>
        </>
      ) : null}
        ...
        <div className="mt-auto pt-4 border-t border-[var(--hairline)] flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-[var(--muted)]">
              Saves {formatCAD(bundleSavings(bundle))}
            </div>
          </div>
          <Link
