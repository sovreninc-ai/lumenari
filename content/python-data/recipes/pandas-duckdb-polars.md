...
The same operations across the three libraries you'll use in a working notebook. Pick by speed, ergonomics, and what the rest of the codebase uses.
...
| Situation | Use |
| --- | --- |
| Default. Small-to-medium DataFrames (< 1M rows). Already on pandas. | **pandas** |
| Complex aggregations, joins, window functions — and SQL would be cleaner | **DuckDB** |
| Working with Parquet files directly without loading into memory | **DuckDB** |
| Million-plus row DataFrames where pandas is getting slow | **Polars** |
| Streaming larger-than-memory data | **Polars (lazy)** |
| You're sharing the notebook with people who don't know Polars | **pandas** |

You can mix all three in one notebook. DuckDB returns pandas DataFrames; Polars converts to pandas with `.to_pandas()` and from pandas with `pl.from_pandas()`.
...
After every join, check row counts. Inner join silently drops unmatched rows.
...
**Recommendation**: window functions are where DuckDB pays for itself. The pandas equivalents work but read worse and are easier to get wrong.
...
## The Polars idiom worth knowing

Polars expressions chain inside `.select()`, `.with_columns()`, `.filter()`, and `.group_by().agg()`. The same expression can be built once and reused.
...
Lazy mode is where Polars really wins for big data:
...
The whole pipeline is optimized as one query. Predicates push down, projections push down, no intermediate DataFrames.

---

## Quick decision tree

- "I just want to look at this dataset" → **pandas**
- "I'm joining 4 tables with a window function" → **DuckDB**
- "I'm querying a folder of Parquet files" → **DuckDB** with `'events/*.parquet'`
- "The DataFrame is 50M rows and pandas is slow" → **Polars**
- "I need to build a one-off report for an exec" → whichever the rest of your code uses
